...
public class GamePlayer extends GameEntity {
    public GamePlayer(int roll, DBUser user, DBGuild guild) {
        ...
        Events.PLAYER_HEALTH.invoke(new EventPlayerHealth(user, guild, health));
        Events.PLAYER_DAMAGE.invoke(new EventPlayerDamage(user, guild, damage));
        Events.PLAYER_BLOCK.invoke(new EventPlayerBlock(user, guild, block));
